...
export function HeavenlyMapWorkspace({
  ...
  const workspaceHeight = compact
    ? Math.max(590, Math.min(730, height - 96))
    : Math.max(620, Math.min(720, height - 150));
  const mobileMapInset = compact && mobileView === 'map'
    ? (selectedRunId ? 194 : 146)
    : 0;
  ...
  return <View style={[styles.section, compact && styles.sectionMobile]}>
    <View style={[styles.topbar, compact && styles.topbarMobile]}>
      <View style={styles.topbarCopy}>
        ...
        <Text style={[styles.title, compact && styles.titleMobile]}>{compact ? `${resortName} mountain map.` : `${resortName}, line by line.`}</Text>
      ...
      {!compact ? <View style={styles.ticket}><Text style={styles.ticketTop}>LOCAL OSM SNAPSHOT</Text><Text style={styles.ticketMain}>MAP / 002</Text></View> : null}
    </View>
    ...
    <View style={[styles.workspace, compact && styles.workspaceMobile, { height: workspaceHeight }]}>
      ...
      <View style={[styles.mapFrame, compact && styles.mapFrameMobile]}>
        ...
        <View style={styles.mapShell}>
          <HeavenlyMap compact={compact} workspace bottomInset={mobileMapInset} selectedRunId={selectedRunId} onSelectRun={onSelectRun} onTerrainAvailabilityChange={setTerrainAvailable} />
          ...
          <View style={[styles.prototypeLabel, compact && styles.prototypeLabelMobile, compact && { bottom: mobileMapInset + 8 }]} pointerEvents="none"><Text style={styles.prototypeText}>PROTOTYPE MAP — TRAIL GEOMETRY FROM OPENSTREETMAP</Text></View>
...
const styles = StyleSheet.create({
  section: { backgroundColor: '#e9e1d3', paddingTop: 16, paddingBottom: 42 },
  sectionMobile: { paddingTop: 12, paddingBottom: 30 },
  topbar: { alignSelf: 'center', width: 'calc(100% - 48px)' as any, maxWidth: 1420, minHeight: 58, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 20, marginBottom: 10 },
  topbarMobile: { width: 'calc(100% - 16px)' as any, minHeight: 50, alignItems: 'center', gap: 8, paddingHorizontal: 4, marginBottom: 6 },
  topbarCopy: { flex: 1, minWidth: 0 },
  eyebrow: { color: colors.orange, fontFamily: fonts.bold, fontSize: 8, letterSpacing: 1.5 },
  title: { color: colors.forest, fontFamily: fonts.display, fontSize: 29, lineHeight: 32, letterSpacing: -.8, marginTop: 2 },
  titleMobile: { fontSize: 24, lineHeight: 27 },
  copy: { color: colors.muted, fontFamily: fonts.body, fontSize: 10, lineHeight: 14, maxWidth: 760, marginTop: 3 },
  ticket: { backgroundColor: colors.orange, borderColor: colors.forest, borderWidth: 1, paddingHorizontal: 10, paddingVertical: 7, transform: [{ rotate: '1.5deg' }] },
  ...
  prototypeLabelMobile: { left: 'auto', right: 7, maxWidth: 168 },
  ...
  mapFooterMobile: { minHeight: 70, paddingHorizontal: 8, paddingVertical: 6 },
